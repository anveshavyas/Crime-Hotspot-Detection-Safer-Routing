...
@app.get("/hotspots/day")
def get_day_hotspots():
    df = load_data()
    day = df[~df["IS_NIGHT"]][["LAT","LON"]].dropna()
    feats = cluster_hotspots(day, K_DAY)
    return {"type":"FeatureCollection","features":feats}

@app.get("/hotspots/night")
def get_night_hotspots():
    df = load_data()
    night = df[df["IS_NIGHT"]][["LAT","LON"]].dropna()
    feats = cluster_hotspots(night, K_NIGHT)
    return {"type":"FeatureCollection","features":feats}
